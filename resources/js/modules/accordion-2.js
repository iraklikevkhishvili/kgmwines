...
/*======================================================
 Observers ---------------------------------------------
=======================================================*/
function bindResizeObservers(getEntries) {
    if (typeof ResizeObserver !== 'function') {
        // Environments without ResizeObserver (older Safari) still need resize sync.
        const onWin = () => {
            getEntries().forEach(({panel}) => {
    ...
    const ensureObservers = () => {
        ...
        getEntries().forEach(({panel}) => {
            ...
            ro.observe(panel);
            observers.set(panel, () => ro.disconnect());
        });

        observers.forEach((dispose, panel) => {
 ...
 * =======================================================*/
function bindClickHandling(root, getEntries) {
    const onClick = (event) => {
...
function bindKeyboardNavigation(root, getEntries) {
 ...
 */
function bindEscapeClose(root, getEntries) {
...
function setupAccordion(root) {
    ...
    let entries = getAccordionEntries(root);
    if (entries.length > 0) {
    ...
    disposers.push(bindClickHandling(root, getEntries));
    disposers.push(bindKeyboardNavigation(root, getEntries));
    disposers.push(bindEscapeClose(root, getEntries));

    const resize = bindResizeObservers(getEntries);
    disposers.push(() => resize.dispose());

    const refreshEntries = () => {
        const next = getAccordionEntries(root);
        if (next.length > 0) {
            setInitialState(root, next);
        }
        entries = next;
        resize.refresh();
    };
